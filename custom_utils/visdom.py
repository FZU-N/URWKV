...
from .preprocessing import numpy_to_torch
...
class VisHeatmap(VisBase):
    ...
    def draw_data(self):
        ...
        # self.visdom.heatmap(self.raw_data.clone(),  opts={'title': self.title, 'colormap': 'jet'}, win=self.title)
        # posMap = self.visdom.heatmap(self.raw_data.clone(),  opts={'title': self.title}, win=self.title)
        # print(posMap)
        # print(type(posMap))
        # plt.imshow(posMap)
...
class VisTracking(VisBase):
    ...
    def save_data(self, data):
        ...
        self.draw_data()   # this line is needed to make draw_data() work

    def draw_data(self):

        disp_image = self.raw_data[0].copy()  # this line is to copy the image whose type is numpy
...
class Visdom:
    def __init__(self, debug=0, ui_info=None, visdom_info=None, port=None):
        self.debug = debug
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                # self.visdom.register(z.view(3, 127,127), 'image', 1, 'Image-z')
                # Tips: the type of inputted 'img' is a tensor with 3 dimensions
                self.registered_blocks[title] = VisImage(self.visdom, show_data, title)
            elif mode == 'heatmap':
                # self.visdom.register(cls[0][1], 'heatmap', 2, 'Cls Map-1')
                # Tip: the inputted heatmap is a tensor with H*W (2 dimensions)
                ...
                # visdom.register((img, pred_bbox), 'Tracking', 1, 'Tracking')
                # Tips: the type of the inputted 'img' is 'numpy', and its shape is H*W*C
                ...
                # self.visdom.register(input_featmap, 'featmap', 1, 'Featmap Map')
                # Tips: the inputted featmap is a tensor
